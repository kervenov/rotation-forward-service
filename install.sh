...
#   • rotation-agent.py + rotation-agent.service (control listener + probe)
#
# The agent sits idle (STANDBY, ~0 CPU/RAM) until the panel POSTs "activate";
# then it ICMP-pings 4 static TM hosts from the current entry IP every
# PROBE_INTERVAL and reports reachability. "deactivate" stops only the probe
# loop — forwarding keeps running. Survives reboot AND crashes
...
#    to DNS/mirror failures. NEVER aborts the install: python3 and ping
#    (iputils-ping) are virtually always preinstalled; both are only installed
...
# ping (iputils-ping) — REQUIRED: the probe model pings TM hosts. Virtually
# always preinstalled; install only if truly missing. The agent runs as root
# so ICMP raw sockets are permitted without extra capabilities.
if ! command -v ping >/dev/null 2>&1; then
  echo "[*] ping yok — iputils-ping kuruluyor..."
  apt_install iputils-ping || true
fi
if ! command -v ping >/dev/null 2>&1; then
  echo "[!] UYARI: ping bulunamadı ve kurulamadı. Ağ/DNS düzelince:"
  echo "    apt-get install -y iputils-ping"
  echo "    Agent yine de kurulacak; ping gelene kadar probe yapamaz ama"
...
[Unit]
Description=Rotation forward-service agent (control listener + reachability probe)
...
[Service]
Type=simple
# Runs as root (no User=) so ICMP probes have raw-socket permission.
...
echo "     • Probe         -> her ~${PROBE_INTERVAL}s, 4 TM host, ${PROBE_COUNT} paket (yalnızca ACTIVE iken)"
echo ""
